...
CREATE OR REPLACE FUNCTION "api"."create_switchport"(input_port_name text, input_system_name text, input_port_type text, input_description text) RETURNS VOID AS $$
	BEGIN
		SELECT api.create_log_entry('API','DEBUG','begin api.create_switchport');
		...
		-- Create directive
		SELECT api.create_log_entry('API','INFO','creating switchport');
...
CREATE OR REPLACE FUNCTION "api"."remove_switchport"(input_port_name text, input_system_name text) RETURNS VOID AS $$
	BEGIN
		SELECT api.create_log_entry('API','DEBUG','begin api.remove_switchport');
		...
		-- Create directive
		SELECT api.create_log_entry('API','INFO','removing switchport');
