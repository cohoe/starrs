...
/* API - create_ip_subnet
	...
	4) Create new subnet
*/
CREATE OR REPLACE FUNCTION "api"."create_ip_subnet"(input_subnet cidr, input_name text, input_comment text, input_autogen boolean, input_dhcp boolean, input_zone text, input_owner text, input_datacenter text, input_vlan integer) RETURNS SETOF "ip"."subnets" AS $$
	DECLARE
		RowCount INTEGER;
	BEGIN
		-- Validate input
	...
	END;
$$ LANGUAGE 'plpgsql';
COMMENT ON FUNCTION "api"."create_ip_subnet"(cidr, text, text, boolean, boolean, text, text, text, integer) IS 'Create/activate a new subnet';
