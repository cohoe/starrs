...
class InterfaceAddress extends ImpulseObject {
	 ...
	 */
	private function _load_pointer_records() {
		$pointer_record_info = $this->CI->api->get_pointer_records($this->address);
		foreach ($pointer_record_info as $record) {
			array_push($this->dnsPointerRecords, new PointerRecord(
				$record['hostname'],
				$record['zone'],
				$record['address'],
				$record['type'],
	...
	/**
	 * Get all of the DNS text records that resolve to this address and put them in the array
	 */
	private function _load_text_records() {
		$text_record_info = $this->CI->api->get_text_records($this->address);
		foreach ($text_record_info as $record) {
			array_push($this->dnsTxtRecords, new TxtRecord(
				$record['hostname'],
				$record['zone'],
				$record['address'],
				$record['type'],
	...
	/**
	 * Get all of the DNS MX records that resolve to this address and put them in the array
	 */
	private function _load_mx_records() {
		$mx_record_info = $this->CI->api->get_mx_records($this->address);
		foreach ($mx_record_info as $record) {
			array_push($this->dnsMxRecords, new MxRecord(
				$record['hostname'],
				$record['zone'],
				$record['address'],
				$record['type'],
	...
	/**
	 * Get all of the DNS NS records that resolve to this address and put them in the array
	 */
	private function _load_ns_records() {
		$ns_record_info = $this->CI->api->get_ns_records($this->address);
		foreach ($ns_record_info as $record) {
			array_push($this->dnsNsRecords, new NsRecord(
				$record['hostname'],
				$record['zone'],
				$record['address'],
				$record['type'],
	...
	/**
	 * Get the DNS A/AAAA record that resolves to this address if it exists
	 */
	private function _load_address_record() {
		$address_record_info = $this->CI->api->get_address_record($this->address);
		if($address_record_info) {
			$this->dnsAddressRecord = new AddressRecord(
				$address_record_info['hostname'],
				$address_record_info['zone'],
				$address_record_info['address'],
				$address_record_info['type'],
				$address_record_info['ttl'],
				$address_record_info['owner'],
				$address_record_info['date_created'],
				$address_record_info['date_modified'],
				$address_record_info['last_modifier']
			);
		}
	}
