...
CREATE TYPE "management"."search_data" AS (datacenter text, availability_zone text, system_name text, location text, asset text, "group" text, platform text, mac macaddr, address inet, config text, system_owner text, system_last_modifier text, range text, hostname varchar(63), cname_alias varchar(63), srv_alias varchar(63), zone text, dns_owner text, dns_last_modifier text);
