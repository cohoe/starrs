...
CREATE OR REPLACE FUNCTION "api"."get_firewall_metahost_members"(input_metahost_name text) RETURNS SETOF INET AS $$
	BEGIN
		RETURN QUERY (SELECT "address" FROM "firewall"."metahost_members" WHERE "name" = input_metahost_name);
	END;
$$ LANGUAGE 'plpgsql';
